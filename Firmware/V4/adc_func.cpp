...
int read_cap_level(uint16_t avg_no) {
	...
	for (uint16_t i = 0; i < avg_no; i++) {
		...
		ADC_set_input_config (ADCCH_POS_PIN0); //connect ADC to ADC3 which is sensor previously charged to VDD.
...
int read_cap_level_ref(uint16_t avg_no) {
	...
	for (uint16_t i = 0; i < avg_no; i++) {
		...
		ADC_set_input_config (ADCCH_POS_PIN1); //connect ADC to ADC2 which is not connected to anything
		_delay_us(10);
